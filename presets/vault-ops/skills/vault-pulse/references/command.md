...
- Attention counts **Claude, Codex and Cortex** on this machine. Cortex is read from `~/Library/Application Support/Cortex Code` (edit history + per-launch `logs/` dirs); its records carry the operator's literal prompts, which the collector reads past and never stores.
- Known bias, by design: **off-tool work is invisible** — meetings, Excel, reading, lectures, anything not done through a tracked agent. Attention is a floor, not a total. The trend stays valid while the tool mix is stable; a new tool with no collector would read as a decline that never happened, so add a collector rather than reinterpreting the dip.
