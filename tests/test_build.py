...
class TestBuildMachinery:
    """A preset's machinery payload (engine + tests) ships junk-stripped."""

    def _write_machinery(self, tmp_repo: Path) -> Path:
        machinery = tmp_repo / "presets" / "python-api" / "machinery"
        ...
        tests_dir = machinery / "tests"
        tests_dir.mkdir()
        (tests_dir / "test_vault_utils.py").write_text("import vault_utils\n")
        return machinery

    def test_build_copies_preset_machinery(self, tmp_repo: Path) -> None:
        self._write_machinery(tmp_repo)

        build_preset("python-api", repo_root=tmp_repo)

        dist_machinery = tmp_repo / "dist" / "python-api" / "machinery"
        assert (dist_machinery / "engine" / "vault_utils.py").exists()
        assert (dist_machinery / "engine" / "queries" / "vault_query.py").exists()
        assert (dist_machinery / "tests" / "test_vault_utils.py").exists()

    def test_build_strips_junk_from_machinery(self, tmp_repo: Path) -> None:
        """Caches regenerated by test runs must never ship into dist."""
        machinery = self._write_machinery(tmp_repo)
        pycache = machinery / "engine" / "__pycache__"
        pycache.mkdir()
        (pycache / "vault_utils.cpython-312.pyc").write_bytes(b"junk")
        pytest_cache = machinery / "tests" / ".pytest_cache"
        pytest_cache.mkdir()
        (pytest_cache / "CACHEDIR.TAG").write_text("junk\n")

        build_preset("python-api", repo_root=tmp_repo)

        dist_machinery = tmp_repo / "dist" / "python-api" / "machinery"
        assert not (dist_machinery / "engine" / "__pycache__").exists()
        assert not (dist_machinery / "tests" / ".pytest_cache").exists()

    def test_build_without_machinery_ships_none(self, tmp_repo: Path) -> None:
        build_preset("python-api", repo_root=tmp_repo)
