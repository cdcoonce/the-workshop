...
Before writing (or trusting) any pressure-scenario test, run it against a subagent with **no skill loaded**. Use `qa-tester`'s scenario-execution mode (`core/agents/qa-tester/AGENT.md`) to dispatch the subagent and capture its choice and rationalization verbatim.
