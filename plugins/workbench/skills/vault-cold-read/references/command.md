...
5. **Unresolvable evidence.** Every path the body names resolves, in either role it plays: as **evidence** (cited as proof of existing state — a file, an issue/PR number, a vault note) or as a **destination** (named as where the slice will create something new — a test file, a module, a fixture).
   → Test: run `uv run --with 'graphmark>=0.7,<0.8' python "<engine>/cold_read_evidence.py" --repo <repo> --repo-dir <local checkout> --issue <N> --json` — one invocation resolves every path, issue/PR number, commit SHA and `[[wikilink]]` the body names, against the target repo's default branch (or a SHA the body pins) and, for wikilinks, the vault. `<engine>` is the same placeholder `vault-audit` and `vault-wrap-up` use — resolve it to a real absolute path before running the command; see `vault-operating-principles.md`. The dispatcher must expand both `<engine>` and the local checkout path into absolute paths in the subagent's procedure text, or the subagent cannot run the script — check the procedure section before dispatching and state explicitly that it did.
   - **BLOCKING rows are detector-5 findings as-is.** Do not re-resolve them by hand — the row already names the token, its class, and why it failed.
   - **JUDGMENT rows need the evidence-vs-destination call, or a comparison of the line text.** That call is the only part left to the reader: a `PARENT_ONLY` path is blocking only if the body cites it as evidence (not a destination); an `ABSENT` symbol may be something this slice is about to create; a `PATH_LINE` that resolves still needs the cited line text compared against the claim — it blocks only when the line no longer supports what the body says it does; a `REFERENCED_ONLY` path is a directory missing from git but named as a string literal in code, typically created at runtime — weigh the evidence line; a bare filename (no directory component, e.g. `tick.py:42-50`) resolves by basename search across the whole tree, and `AMBIGUOUS_BASENAME` needs the reader to pick the intended file among the candidates listed; a `{...}` filename convention with no directory before the placeholder (e.g. `day-{day:04d}.md`) is counted (`TEMPLATED_NO_PREFIX`), not resolved — there is nothing to check.
   - **`INCOMPLETE`:** the classes listed under `not_run` (no gh auth, fetch failed, missing graphmark with wikilinks present, a bad `--repo`/`--repo-dir`/`--ref`) are resolved by hand, the old one-unpiped-command-each way, for those classes only.
   - A citation or destination that does not resolve is either stale, invented, or the wrong path; all three are blocking (precedent: #568). Example: an AC saying a test lands in `scripts/tests/` is a destination claim — that path resolves only if `scripts/tests/` exists at repo root. It doesn't; the directory shape it's borrowing lives one level down inside individual skills (`plugins/workbench/skills/adversarial-review/scripts/tests/`, among others), and the repo's real suite is `tests/`, which does exist and is the parent the AC should have named.
...
3. **Resolve every path the body names** — evidence and destination alike (detector 5) — with the single `cold_read_evidence.py` invocation described there. Its BLOCKING rows are findings as-is; its JUDGMENT rows still need your read. Only a class listed under its `not_run` falls back to one unpiped command each.
4. **Assign a verdict:**
