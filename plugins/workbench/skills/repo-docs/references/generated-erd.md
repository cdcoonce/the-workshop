...
Building the generator touches files outside `docs/` (the script, the sidecar, drift tests, `.prettierignore`): propose it as its own change and get approval first.
...
When you skip, say so in one sentence in `data-flow.md` ("Entities are described in prose; the repo declares no SQL DDL"). Never create a landing page to hold the note.
...
The page still carries the skill's provenance footer (`mode=reference`), **outside** the end marker,
...
invisible to it. Give the page a short "Provenance" section stating that limit, and link a how-to for verifying against <engine>
that:
