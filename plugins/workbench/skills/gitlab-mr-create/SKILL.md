...
Before creating the MR, the script runs the `mr-preflight` reference sweep from the merge-base with `origin/dev`. It refuses while any reference to an identifier the branch renamed survives at `HEAD` unwaived outside the paths the repo's `.mr-preflight.toml` ignores, while the description holds a malformed `- waive` line, when `origin/dev` is missing, and when that committed `.mr-preflight.toml` is unusable (every `dev` MR in the repo, until it is fixed). Each hit prints as `path:line: OLD (renamed to NEW in FILE)`; fix it and commit, or, for an intentional old name such as a changelog entry, add `- waive OLD path: reason` to the description's sweep block, then run the script again. Permanent noise the whole repo ignores, such as every changelog hit, belongs in a committed `.mr-preflight.toml` instead. A passing sweep still prints its output on stderr: waived hits, and how many hits that file suppressed. When the branch renamed something, the MR's description carries the rendered sweep block after your prose: what was renamed, and every waiver with its reason; otherwise the description goes out exactly as written. Your description file itself is never rewritten. The `mr-preflight` skill explains what counts as a rename and how waivers work.
