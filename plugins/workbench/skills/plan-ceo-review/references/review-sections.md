# Plan Review — Detailed Procedure

This reference holds the deep procedure the invocation-loaded `SKILL.md`
points into: engineering preferences, the deep pre-review grounding pass,
the full Step 0 scope challenge, the mode comparison table, and the ten
review sections. Load the section named in the pointer that sent you here.
...
## Pre-Review Grounding — Grounding, Retrospective, and Taste Checks

Load this when the plan is large, spans a multi-document set, or the docs may contradict each other. It supplements the base system audit run at the start of every review (git log/diff/stash, TODO/FIXME grep, recently modified files, CLAUDE.md, `docs/plans/`).
...
Execute these sections after scope and mode are agreed in Step 0.
...
- **Rollback posture.** If this ships and immediately breaks, what's the rollback procedure? Git revert? Feature flag? Schema migration rollback? How long?
...
```
  ...
  -------------------------|-----------------------------|-----------------
  DataService.fetch()      | API timeout                 | TimeoutError
...
- Catching a broad exception with only `logger.error(e)` is insufficient. Log the full context: what was being attempted, with what arguments, for what user/request.
...
- For each GAP (unhandled error that should be handled): specify the handling action and what the user should see.
...
- **Query optimization.** For every new database query or ORM traversal: is there eager loading? Are queries batched? Any N+1 patterns?
- **Memory usage.** For every new data structure: what's the maximum size in production?
- **Database indexes.** For every new query: is there an index?
- **Caching opportunities.** For every expensive computation or external call: should it be cached?
- **Background job sizing.** For every new job: worst-case payload, runtime, retry behavior?
- **Slow paths.** Top 3 slowest new codepaths and estimated p99 latency.
- **Connection pool pressure.** New DB connections, cache connections, HTTP connections?
...
- **Admin tooling.** New operational tasks that need admin UI or CLI commands?
...
- **Migration safety.** For every new schema migration: backward-compatible? Zero-downtime? Table locks?
