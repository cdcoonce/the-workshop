...
**Human-launched only.** You typing `/connect` _is_ the sanctioned run (Constitution §2: "inside a
...
/connect --term <term>  # manual mode — target notes matching <term> instead of a graph gap scan
```
...
1. **Candidates.** Run `uv run "<engine>/graph_cli.py" --gaps --top <N>` (default N=12) — or, in
   ...
   **structurally-siloed** notes (those reachable only via a single bridge — the under-integrated
   periphery). Same loop either way; bridges only _target_ where to look, never force a link. These are
   ...
   In **`--term <term>` mode**, skip the graph scan: search note filenames, frontmatter descriptions,
   and note content for `<term>`, case-insensitive. Every unlinked pair among the matching notes becomes
   a candidate for the same drafting pass below — term only _targets_ where to look, same as bridges; it
   never forces a link either. If nothing matches `<term>`, don't fall through to an empty batch: report
   that no note covers it yet and propose creating one, naming the folder its content best fits.
...
5. **Resolve + report.**
   - **Deselected** pairs → run `uv run "<engine>/graph_cli.py" --dismiss "<a>" "<b>"` per pair.
     ...
     — it auto-expires and the pair re-surfaces if _either note later changes_, so a "no" is only as
...
- **One gate, not none** — the batch preview (step 3) is mandatory; nothing is written before Charles
  confirms. Autonomy is in the _drafting_, not the _landing_.
- **Reuse, don't rebuild** — candidates come from `graph_cli --gaps`, or a filename/frontmatter/content
  search in `--term` mode; dismissals are **content-aware** (`graph_cli --dismiss`, fingerprinted in
  `connect-dismissed.json`) so they expire when notes evolve. No new analysis engine.
