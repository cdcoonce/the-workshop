...
5. **File it.** `gh issue create --repo <repo> --title "<imperative title>" --label "proposed,afk-sized"` (epics: `proposed,decompose:ready`). Add `afk:cheap` or `afk:frontier` when the tier call is clear — only if the repo has those labels (the-workshop doesn't; the Budget line carries the tier there).
   ...
   - **Children wait for the whole set.** In either flavor, no child is promoted, whether by `--promote <child>` or `--promote-epic`, until its own cold read has cleared and the parent carries a `## Coverage — CLEAN` record from the /cold-read [coverage pass](../../vault-cold-read/references/command.md#the-coverage-pass-decomposed-parents). That pass reads the child set once and withholds every child while any parent criterion, or the parent's anti-scope, Budget or gate command, is missing from the set.
