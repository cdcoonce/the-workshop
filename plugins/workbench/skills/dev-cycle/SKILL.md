...
**Disambiguation:** If the user only wants a PRD, route to `/write-a-prd`. If they only want a plan, route to `/prd-to-issues --plan`. If the idea is still fuzzy — no committed solution yet — run `brainstorm` first; its brief seeds Phase 1. This skill is for the full end-to-end lifecycle.
...
| 2   | **Plan**        | `prd-to-issues --plan`                                 | Plan file exists at `docs/plans/` |
